...
#define TCS_SDA_READ  HAL_GPIO_ReadPin(SDA_PORT,SDA_PIN)

#define TCS_SCL_H     HAL_GPIO_WritePin(SCL_PORT, SCL_PIN, 1)
#define TCS_SCL_L     HAL_GPIO_WritePin(SCL_PORT, SCL_PIN, 0)
#define TCS_SDA_H     HAL_GPIO_WritePin(SDA_PORT, SDA_PIN, 1)
#define TCS_SDA_L     HAL_GPIO_WritePin(SDA_PORT, SDA_PIN, 0)
...
//模块操作函数
void TCS34725_SetIntegrationTime(uint8_t time);//设置转换的时间（时间越长,RGB数值越大）
void TCS34725_SetGain(uint8_t gain);//设置亮度的增益
void TCS34725_Enable(void);//模块使能
void TCS34725_Disable(void);//模块失能
uint8_t TCS34725_Init(void);//模块初始化
void TCS34725_SetAIHT(uint16_t set);//设置中断上限
void TCS34725_SetAILT(uint16_t set);//设置中断下限
uint16_t TCS34725_GetChannelData(uint8_t reg);//获取寄存器数据
uint8_t TCS34725_GetRawData(COLOR_RGBC *rgbc);//获取RGBC数值
